...
---
//Moore file mappings updated by AJS 2023_12_17 712PM,
...
payload map (item,index) -> ({
    ...
    acquired_batch_number: item.batch_upload_batch default null,
    acquired_batch_sequence: item.batch_upload_sequence default null,
    ...
    credit_soft_credit_type: item.credit_1_soft_credit_type default null,
    ...
    account_1_name: if(lower(item.account_record_type) =="organization") item.contact_1_last_name  else null, 
    ...
    contact_1_phone_type: item.contact_1_phone_type default null,
    contact_1_email: item.contact_1_email default null,
    contact_1_email_type: item.contact_1_email_type default null,
    ...
    donation_name:
        if(item.giving_record_type == "GF" and item.giving_giving_frequency == "One Payment") "Moore One Time Gift" else null,
    giving_tax_entity:
    ...
    giving_gau_debit_code: 
        (item.file_name) match {
            case file if((upper(file) contains 'C3') and ((lower(item.payment_method_payment_type) == "charge card") or (lower(item.payment_method_payment_type) == "eft"))  )  -> 'C312800'
            case file if((upper(file) contains 'C3')) -> 'C312615'
            case file if((upper(file) contains 'C4') and ((lower(item.payment_method_payment_type) == "charge card") or (lower(item.payment_method_payment_type) == "eft")) )-> 'C412800'
            case file if((upper(file) contains 'C4')) -> 'C412625'
            case file if((upper(file) contains 'VOTES') and ((lower(item.payment_method_payment_type) == "charge card") or (lower(item.payment_method_payment_type) == "eft")) )-> 'PV12800'
            case file if((upper(file) contains 'VOTES')) -> 'PV12640'
            case file if((upper(file) contains 'PAC') and ((lower(item.payment_method_payment_type) == "charge card") or (lower(item.payment_method_payment_type) == "eft")) )-> 'PC12800'
            case file if((upper(file) contains 'PAC')) -> 'PC12630'
            else -> null
        },
    giving_record_type: item.giving_record_type,
    ...
    //giving_comments: item.giving_comments default null,
    giving_close_date: 
        if(item.giving_record_type == "GF" or isEmpty(item.giving_record_type) ) item.giving_close_date else null,
    
    
    donation_stage: if(item.payment_method_payment_type != "Cash/Check") "Active" else "Closed/Won",

    //opportunity_vendor_pledge_number: if(item.giving_record_type == "GF" or isEmpty(item.giving_record_type) ) item.giving_giving else null,        
    payment_paid: if(item.payment_method_payment_type == "Cash/Check") true else false,
    ...
    payment_method_account_number: 
        if(item.payment_method_account_number != null) item.payment_method_account_number else item.payment_method_account default null,
    payment_method_routing_number: 
        if(item.payment_method_routing_number != null) item.payment_method_routing_number else item.payment_method_routing default null,
    payment_check_number: item.giving_check default null,
    payment_check_date: item.giving_check_date default null,
    ...
    recurring_donation_effective_date: 
        if(item.giving_record_type == "PL" and item.giving_giving_frequency == "Monthly") item.giving_close_date else null,
   // recurring_donation_vendor_pledge_number: if((item.giving_record_type == "PL" or isEmpty(item.giving_record_type)  ) and item.giving_giving_frequency == "Monthly") item.giving_giving else null,
    giving_recurring_amount:
        if(item.giving_record_type == "PL" and item.giving_giving_frequency == "Monthly") item.giving_giving_amount as Number else null,
    recurring_donation_installment_period:
        if(item.giving_record_type == "PL" and item.giving_giving_frequency == "Monthly") "Monthly" else null,
    recurring_donation_installment_frequency:
        if(item.giving_record_type == "PL" and item.giving_giving_frequency == "Monthly") 1 else null,

    recurring_donation_payment_method: if(item.giving_record_type == "PL" and item.giving_giving_frequency == "Monthly") (item.payment_method_payment_type match {
        case "Charge Card" -> "Credit Card"
        case "EFT" -> "ACH"
        else -> item.payment_method_payment_type
    }) else null,
    recurring_donation_date_established: if(item.giving_record_type == "PL" and item.giving_frequency == "Monthly") item.giving_close_date else null,
    recurring_donation_recurring_type:
        if(item.giving_record_type == "PL" and item.giving_giving_frequency == "Monthly") "Open" else null,
    recurring_donation_day_of_month:
        if(item.giving_record_type == "PL" and item.giving_giving_frequency == "Monthly") "1" else null,
    recurring_donation_name:
        if(item.giving_record_type == "PL" and item.giving_giving_frequency == "Monthly") "Moore Recurring Donation" else null,
    recurring_donation_status:
        if(item.giving_record_type == "PL" and item.giving_giving_frequency == "Monthly") "Active" else null,
   ...
   // response_category: item.response_category default null,
